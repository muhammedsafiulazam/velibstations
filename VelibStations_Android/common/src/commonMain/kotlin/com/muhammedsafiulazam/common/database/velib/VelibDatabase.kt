...
class VelibDatabase(val db: VelibDB) : IVelibDatabase {
    override fun getRecords(latitude: Double, longitude: Double, radius: Double)  {
        GlobalScope.launch(CouroutineUtils.DISPATCHER) {
            ...
            val event = Event(VelibDatabaseEventType.GET_RECORDS, records, null)
            Knowledge.getEventManager().send(event)
        }
    }

    override fun addRecords(records: ArrayList<Record>?) {
        GlobalScope.launch(CouroutineUtils.DISPATCHER) {
            records?.forEach { record ->
                addRecord(record)
            }

            val event = Event(VelibDatabaseEventType.ADD_RECORDS, records, null)
            Knowledge.getEventManager().send(event)
        }
    }
    ...
    override fun cleanRecords() {
        GlobalScope.launch(CouroutineUtils.DISPATCHER) {
            ...
            val event = Event(VelibDatabaseEventType.CLEAN_RECORDS, null, null)
            Knowledge.getEventManager().send(event)
        }
    }
